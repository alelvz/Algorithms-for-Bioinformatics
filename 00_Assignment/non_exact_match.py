...
def fuzzy_pattern_matching(genome, alu_seq):
    """Finds matches of AluY in a given genome sequence allowing up to one mismatch."""
    alu_len = len(alu_seq)
    matches = []

    for i in range(len(genome) - alu_len + 1):
        segment = genome[i:i+alu_len]
        mismatches = 0
        j = 0
        k = 0

        while j < alu_len and k < len(segment):
            if alu_seq[j] != segment[k]:
                mismatches += 1
                if mismatches > 1:
                    break  # Stop checking if more than one mismatch
                # Try skipping one character (deletion in genome)
                if k + 1 < len(segment) and alu_seq[j] == segment[k + 1]:
                    k += 1  # Skip one character in genome
                # Try skipping one character in the AluY sequence (insertion in genome)
                elif j + 1 < alu_len and alu_seq[j + 1] == segment[k]:
                    j += 1  # Skip one character in AluY
            j += 1
            k += 1
...
def main(genome_file, alu_file):
    """Main function to run pattern matching."""
    ...
    for chrom, sequence in genome_sequences.items():
        ...
        matches = fuzzy_pattern_matching(sequence, alu_seq)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python non_exact_match.py <genome_fasta.gz> <alu_fasta>")
        sys.exit(1)
