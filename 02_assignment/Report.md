...
The assembly was run using both **HiFi long reads** and **Hi-C sequencing data**. The resulting files are stored in the directory `results/hifiasm/`. Among the multiple outputs generated by Hifiasm, the primary file for evaluation is: `lizard_asm_hic.hic.p_ctg.gfa`. This file represents the **primary contigs** (`p_ctg`) of the haploid genome assembly, incorporating the Hi-C phasing information (`hic`) and includes sequences in GFA format. For downstream evaluations such as QUAST, BUSCO, and Merqury, this file was converted to a FASTA file using the following command 

`gfatools gfa2fa $ASSEMBLY_GFA > $ASSEMBLY_FASTA`

The entire workflow for the evaluation is in the file `scripts/evaluate_lizard_assembly.sh` and the results are in `results/hifiasm_evaluation/` in the subfolder `quast`, `busco`, `merqury`, and `flagger`.
