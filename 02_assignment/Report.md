...
### 2️⃣ Gene completeness (BUSCO)

To assess gene content completeness, we ran **BUSCO v5.8.3** on the Hifiasm assembly (`lizard_p_ctg.fasta`) using the **sauropsida_odb10** lineage dataset, which includes 7,480 conserved orthologs from reptiles and birds. The analysis identified a total of **98.2% complete BUSCOs**, of which **97.4% were single-copy** and **0.8% duplicated**, indicating highly complete and mostly non-redundant gene content. Only **0.2% were fragmented** and **1.5% were missing**, demonstrating minimal loss or fragmentation of conserved genes. These results confirm that the genome assembly captures nearly the entire expected gene set for sauropsids, supporting its use in downstream functional annotation and evolutionary analysis.
